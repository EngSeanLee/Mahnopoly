...
export async function updateSession(request: NextRequest) {
  ...
  // supabase/schema.sql). src/app/admin/(protected)/layout.tsx repeats
  // this check server-side too — this one just avoids the extra round
  // trip for the common case of no session at all.
  let isStaff = false;
  if (user?.email) {
